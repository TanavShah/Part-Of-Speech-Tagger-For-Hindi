...
class HindiTagger:
    ...
    def data_tuples(self, file_name):
        vakya_list = []
...
def main():
    ...
    train_data = tagger.data_tuples(tagger.train_file_name)
    test_data = tagger.data_tuples(tagger.test_file_name)
    ...
    t_train = [tup for tup in (sent for sent in training_set)]
    t_test = [tup for tup in (sent for sent in testing_set)]
    
    featuresetsX = [(word_features(n), gender) for (n, gender) in t_train[0]]
    featuresetsY = [(word_features(n), gender) for (n, gender) in t_test[0]]


    classifier = NaiveBayesClassifier.train(featuresetsX)

    print("Classifier accuracy percent:",(accuracy(classifier, featuresetsY))*100)
